...
float sum;
float M1_lastspeed = 0;
float M2_lastspeed = 0;
  ...
  while(1){
    ...
    if(sum == 0){
      ...
      if(sum == 0){
        if(turn != 's'){
          motor(0, 0);
          (turn == 'r') ? motor(Pt, -Pt) : motor(-Pt, Pt);
          while(!IR[2] ) scanIR();
            turn = 's';
        }
      }
    ...
    M1_speed = Pt + Turn - abs(error) * 7;//7
    M2_speed = Pt - 8 - Turn - abs(error) * 7;//7
    if(!IR[1] && IR[2] && !IR[3] ) {
      delay(0);
      if(!IR[1] && IR[2] && !IR[3] ){
        M1_speed = M1_speed * 1.4, M2_speed = M2_speed * 1.4;
      }
    }
...
void loop(void){
  ...
  if(butCnt > 1){
    scanIR();
    PID(30, 0.8, 40, 180);//PID(46, 0.8, 40, 180);
  }
}
//==============MAIN======================================================================================================================================
